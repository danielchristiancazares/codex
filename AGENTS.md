...
- `origin` is the personal repository; `upstream` is OpenAI's synchronization source.
  ...
  commits onto operator-selected tagged OpenAI versions (`rust-v*`), including
  alpha, beta, and RC tags. `upstream/main` is for reference. Use the recorded
  upstream base and explicit `git rebase --onto` procedure in
  `docs/fork-workflow.md` so only fork commits are replayed.
