...
fn response_created_receipt_is_runtime_only_and_redacted() {
    ...
    .to_string();
    let event: ResponsesStreamEvent = serde_json::from_str(&raw_event).unwrap();
    let ResponseEvent::Created { guardian_ticket } =
        process_responses_event(event, &raw_event).unwrap().unwrap()
    else {
...
fn missing_or_malformed_response_receipts_do_not_become_tickets() {
    for response in [
        ...
        let raw_event = json!({"type": "response.created", "response": response}).to_string();
        let event = serde_json::from_str(&raw_event).unwrap();
        assert!(matches!(
            process_responses_event(event, &raw_event).unwrap(),
