...
///
/// The transport deadline is refreshed by any inbound frame, including Ping/Pong control
/// frames the pump answers without forwarding. The progress deadline is refreshed only by
/// acknowledged response events, so a healthy socket cannot hide a permanently stalled
/// response and a delayed-but-live response is not abandoned because control frames were
