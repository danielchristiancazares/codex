    ...
    loop {
        ...
        match message {
            Message::Text(text) => {
                ...
                // Wrapped `{"type":"error",...}` frames are only re-parsed when the decoded
                // kind says so, or when the ordinary event shape did not decode at all.
                ...
                let event = match event_result {
                    Ok(event) => event,
                    Err(err) => {
                        // A frame that is not even valid JSON cannot be recovered by
                        // reading further; fail the response with bounded diagnostics.
