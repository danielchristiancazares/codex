...
async fn exercise_console_policy(name: &str) -> anyhow::Result<()> {
    ...
    if std::env::var_os(PARENT).is_none() {
        ...
        parent
            .args([
                "--exact",
                &format!("engine::command_runner::windows_console_tests::{name}"),
                "--nocapture",
            ])
        ...
        assert!(
            ...
            "detached hook regression failed: {output:?}"
        );
    ...
    let (stdout, stderr) = match name {
        ...
        "uncontained_command_hook_has_no_console" => {
            let output =
                super::build_command(&shell, "--test-threads=1", &environment, &HashMap::new())
                    .kill_on_drop(true)
                    .output()
                    .await?;
        ...
        "legacy_notification_has_no_console" => {
            ...
            let mut command =
                crate::registry::command_from_argv(&argv, environment.iter().cloned())
    ...
    assert!(
        stdout.contains("hook-console=none"),
        "hook acquired a console: {stdout}"
    );
