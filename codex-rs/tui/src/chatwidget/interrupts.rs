...
impl QueuedInterrupt {
    ...
    pub(crate) fn starts_groupable_mcp_call(&self) -> bool {
        matches!(
            self,
            QueuedInterrupt::ItemStarted(ThreadItem::McpToolCall { server, .. })
                if !crate::history_cell::is_computer_activity_server(server)
        )
    }
