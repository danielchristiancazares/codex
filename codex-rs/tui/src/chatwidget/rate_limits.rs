    ...
    fn on_rate_limit_snapshot_from(
        ...
        if let Some(mut snapshot) = snapshot {
            ...
            match source {
                ...
                RateLimitSnapshotSource::RollingUpdate => {
                    if let Some(current) = self.rate_limit_snapshots_by_limit_id.get_mut(&limit_id)
                    {
                        current.credits = display.credits;
                        current.individual_limit = display.individual_limit;
                        current.workspace_access = display.workspace_access;
                    }
                }
            }
