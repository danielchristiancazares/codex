...
#[tokio::test]
async fn model_catalog_default_service_tier_respects_explicit_opt_out() {
    ...
    chat.config.service_tier = ServiceTier::Default;
    chat.local_settings.notices.fast_default_opt_out = None;
    chat.set_feature_enabled(Feature::FastMode, /*enabled*/ true);

    assert_eq!(chat.current_service_tier(), ServiceTier::Fast);

    chat.local_settings.notices.fast_default_opt_out = Some(true);
    chat.refresh_effective_service_tier();

    assert_eq!(chat.current_service_tier(), ServiceTier::Default);
}
