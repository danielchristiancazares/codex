//! Narrow menus show the selected description below a single-column list. Wider menus wrap
//! each description beneath its own row, so every visible choice has readable help.
...
use crate::bottom_pane::selection_popup_common::measure_rows_height_with_col_width_mode;
...
impl CommandPopup {
    fn description_rows(&self, width: u16) -> u16 {
        ...
        textwrap::wrap(
            ...
            textwrap::Options::new(usize::from(width.max(1)))
                .initial_indent("  ")
                .subsequent_indent("  "),
        )
        .len()
    ...
    /// Measure the list and its optional detail area. The composer stays anchored below both.
    pub(crate) fn calculate_required_height(&self, width: u16) -> u16 {
        ...
        items
            ...
            .saturating_add(u16::from(description > 0))
    }
}
...
impl WidgetRef for CommandPopup {
    fn render_ref(&self, area: Rect, buf: &mut Buffer) {
        ...
        let description_height = self
            .description_rows(area.width)
            .min(area.height.saturating_sub(2));
        let list_height = area
            .height
            .saturating_sub(description_height)
            .saturating_sub(u16::from(description_height > 0));
        ...
        let options = textwrap::Options::new(usize::from(area.width))
            .initial_indent("  ")
            .subsequent_indent("  ");
        let wrapped = textwrap::wrap(selected.description(), options);
        for (row, text) in wrapped
            .iter()
            .take(usize::from(description_height))
            .enumerate()
        {
            let text = if row + 1 == usize::from(description_height)
                && wrapped.len() > usize::from(description_height)
            {
                format!("{text}…")
            } else {
                text.to_string()
            ...
            truncate_line_with_ellipsis_if_overflow(
                Line::from(Span::styled(text, secondary_style())),
                usize::from(area.width),
            )
            .render(
                Rect::new(
                    area.x,
                    list_area.bottom() + 1 + row as u16,
                    area.width,
                    /*height*/ 1,
                ),
                buf,
