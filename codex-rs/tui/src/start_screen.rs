...
mod identity;
pub(crate) use identity::StartScreenIdentity;

const COLUMN_WIDTH: u16 = 88;
const COMPOSER_MIN_HEIGHT: u16 = 6;

#[derive(Clone, Copy, PartialEq, Eq)]
pub(crate) enum StartScreenPresentation {
    Introduction,
    Selection,
    /// Completion results grow upward from a stable bottom-docked search field.
    Completion,
}

pub(crate) struct StartScreen<'a> {
    pub presentation: StartScreenPresentation,
...
impl StartScreen<'_> {
    ...
    fn layout(&self, area: Rect) -> (Rect, Rect, Rect) {
        ...
        let composer_height = self
            ...
            .max(match self.presentation {
                StartScreenPresentation::Introduction => COMPOSER_MIN_HEIGHT,
                StartScreenPresentation::Selection | StartScreenPresentation::Completion => 0,
            })
            .min(area.height);
        let header_height = if self.presentation != StartScreenPresentation::Introduction {
            0
        } else if area.height >= composer_height.saturating_add(12) {
            9
        ...
        let actions_height = if header_height > 1
            && area.height
                >= composer_height
        ...
        let y = area.y
            + match self.presentation {
                StartScreenPresentation::Introduction | StartScreenPresentation::Selection => {
                    area.height.saturating_sub(total_height) / 2
                }
                StartScreenPresentation::Completion => area.height.saturating_sub(total_height),
            };
...
impl Renderable for StartScreen<'_> {
    ...
    fn desired_height(&self, width: u16) -> u16 {
        self.composer
            ...
            .saturating_add(match self.presentation {
                StartScreenPresentation::Introduction => 13,
                StartScreenPresentation::Selection | StartScreenPresentation::Completion => 0,
            })
    }
