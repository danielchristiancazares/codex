...
impl App {
    ...
    pub async fn run(
        ...
        let collaboration_modes = bootstrap.collaboration_modes.clone();
        ...
        let mut model_catalog = Arc::new(
            ModelCatalog::new(available_models.clone())
                .with_collaboration_modes(collaboration_modes.clone()),
        );
                ...
                let resumed = if let Some(response) = startup_resume_response.take() {
                    match startup_draft
                        .run_until(tui, async {
                            ...
                            app_server
                                .complete_resume_thread(response, &local_settings, &config)
                                .await
                        })
                        .await
                    {
...
See the Codex keymap documentation for supported actions and examples."
        ...
        if (requires_openai_auth && has_chatgpt_account) || app.config.model_provider.is_copilot() {
