...
#[tokio::test]
async fn ordinary_resumes_preserve_models_absent_from_the_picker_catalog() -> Result<()> {
    ...
    for mode in [ThreadParamsMode::Embedded, ThreadParamsMode::Remote] {
        ...
        let (mut server, requests, proxy) = match mode {
            ...
            ThreadParamsMode::Remote => start_recording_remote_app_server(&app.config).await?,
        };
        let mut session = test_thread_session(ThreadId::new(), app.config.cwd.to_path_buf());
        session.model = "model-not-in-the-picker".to_string();
        session.model_provider_id = match mode {
            ThreadParamsMode::Embedded => app.config.model_provider_id.clone(),
            ThreadParamsMode::Remote => "provider-defined-only-on-the-server".to_string(),
        };
        let mut config = app.config.clone();
        let mut models = app.model_catalog.try_list_models().unwrap_or_default();
        ...
        assert_eq!(models, original_models);
        assert!(recorded_params(&requests, "model/list").is_empty());
...
"#,
    ...
    assert_eq!(
        (
            ...
            resumed_config.model_provider.base_url.as_deref(),
            resumed_models
                .iter()
