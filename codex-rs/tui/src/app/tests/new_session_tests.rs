...
async fn replacement_uses_server_defaults_and_preserves_explicit_launch_settings() -> Result<()> {
    for (managed_effort, explicit, expected_model, expected_effort) in [
        (None, "saved", "gpt-5.6-terra", "high"),
        (Some("medium"), "model", "gpt-5.5", "high"),
        (Some("medium"), "effort", "gpt-5.6-terra", "low"),
        (None, "profile_model", "gpt-5.2", "high"),
        (None, "profile_effort", "gpt-5.6-terra", "low"),
        (Some(""), "profile", "gpt-5.6-luna", "low"),
        (Some("medium"), "profile", "gpt-5.6-luna", "medium"),
    ] {
        ...
        match explicit {
            "model" => app.harness_overrides.model = Some("gpt-5.5".to_string()),
        ...
        if explicit == "saved" {
            ...
            let project = rendered
                ...
                .expect("footer project path");
            let rendered = rendered.replace(&project, "<PROJECT>");
            insta::assert_snapshot!(rendered, @r"
            ────────────────────────────────────────────────────────────────────────────────
            │›
            ────────────────────────────────────────────────────────────────────────────────
              GPT 5.6 Terra High · <PROJECT>
            ");
        }
