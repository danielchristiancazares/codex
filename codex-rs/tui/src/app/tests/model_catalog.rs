...
#[tokio::test]
async fn model_picker_refresh_uses_the_active_thread_provider() -> Result<()> {
    let (mut app, mut events, _ops) = make_test_app_with_channels().await;
    let (mut server, requests, proxy) =
        backend_banner_fallback_tests::start_fallback_thread(&mut app).await?;
    let selected_provider = app.chat_widget.config_ref().model_provider_id.clone();
    app.config.model_provider_id = "unselected-test-provider".to_string();
    ...
    app.handle_event(&mut tui, &mut server, request).await?;
    let loaded = tokio::time::timeout(Duration::from_secs(/*secs*/ 10), async {
        loop {
            if let Some(event @ AppEvent::ModelsLoaded { .. }) = events.recv().await {
                break event;
            }
        }
    })
    .await?;
    assert_matches!(loaded, AppEvent::ModelsLoaded { result: Ok(_), .. });
    let sent = requests.lock().unwrap().clone();
    let model_request = sent
        .iter()
        .find(|request| request.method == "model/list")
        .expect("forwarded model catalog request");
    assert_eq!(
        serde_json::from_value::<codex_app_server_protocol::ModelListParams>(
            model_request.params.clone().expect("model request params")
        )?,
        codex_app_server_protocol::ModelListParams {
            model_provider: Some(selected_provider),
            cursor: None,
            limit: None,
...
async fn model_picker_refresh_updates_app_catalog_from_app_server() -> Result<()> {
    ...
    for model in models.iter_mut() {
        ...
        model.default_service_tier = if model.is_default {
            ServiceTier::Fast
        } else {
            ServiceTier::Default
        };
    ...
    for (model, expected_tier) in [
        (None, ServiceTier::Fast),
        (Some("gpt-5.2"), ServiceTier::Default),
    ] {
        config.model = model.map(str::to_string);
        let started = app_server.start_thread(&config).await?;
        assert_eq!(started.session.service_tier, expected_tier);
    }
