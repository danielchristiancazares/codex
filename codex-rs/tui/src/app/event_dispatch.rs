...
&self.local_settings,
            ...
            AppEvent::ApplyAdvancedReasoning { model, effort } => {
                match model_selection::ModelSelection::new(
                    &model,
                    effort,
                    model_selection::ModelSelectionScope::Global,
                ) {
                    Ok(selection) => {
                        self.commit_model_selection(
                            app_server,
                            selection.commit(model_selection::ContextWindowSelection::default()),
                        )
                        .await;
                    }
            ...
            AppEvent::OpenWindowsSandboxFallbackPrompt {
                ...
                self.show_windows_sandbox_fallback(origin_thread_id, preset, profile_selection);
            }
            AppEvent::BeginWindowsSandboxElevatedSetup {
                preset,
                profile_selection,
            } => {
                self.begin_windows_sandbox_elevated_setup(preset, profile_selection).await;
            }
            AppEvent::BeginWindowsSandboxLegacySetup {
                preset,
                profile_selection,
            } => {
                self.begin_windows_sandbox_legacy_setup(preset, profile_selection).await;
            }
            AppEvent::EnableWindowsSandboxForAgentMode {
                ...
                self.enable_windows_sandbox_for_agent_mode(
                    app_server,
                    origin_thread_id,
                    preset,
                    mode,
