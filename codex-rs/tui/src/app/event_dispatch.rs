...
&self.local_settings,
            ...
            AppEvent::PersistServiceTierSelection { service_tier } => {
                let fast_default_opt_out = service_tier.is_default();
                self.refresh_status_line();
                self.config.service_tier = service_tier;
                self.config.notices.fast_default_opt_out = Some(fast_default_opt_out);
                self.local_settings.notices.fast_default_opt_out = Some(fast_default_opt_out);
                self.chat_widget
                    .local_settings
                    .notices
                    .fast_default_opt_out = Some(fast_default_opt_out);
                self.sync_active_thread_service_tier_to_cached_session()
                    .await;
                let edits = vec![
                    crate::config_update::service_tier_selection_edit(service_tier),
                    crate::config_update::replace_config_value(
                        "notice.fast_default_opt_out",
                        serde_json::json!(fast_default_opt_out),
                    ),
                ];
                match self
