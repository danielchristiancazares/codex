...
fn semantic_exploration_group_remains_semantic_beyond_four_calls() {
    ...
    assert_eq!(
        ...
        vec![
            "• Explored",
            "  └ Searched query-1 in codex-rs/tui, query-2 in codex-rs/tui, query-3 in codex-rs/tui, query-4 in codex-rs/tui, query-5 in codex-rs/tui",
        ]
    );
...
#[test]
fn command_outcomes_keep_ansi_color_and_readable_completion_emphasis() {
    let presentations = [
    ...
    .map(|state| {
        let cell = ExecCell::new(
            roster_call("verification", "just test -p atlas-client", state),
            /*animations_enabled*/ false,
        );
        (state, cell.display_lines(/*width*/ 80))
    });
    insta::assert_debug_snapshot!("command_outcome_readability", presentations);
}
