...
fn group_rendering_snapshots_cover_lifecycle_states() {
    ...
    insta::assert_snapshot!(&rendered(one.display_lines(/*width*/ 80)), @r#"
    ...
    insta::assert_snapshot!(&rendered(group.display_lines(/*width*/ 80)), @r#"
    • Calling server.lookup({"title":"A"})
    ...
    insta::assert_snapshot!(&rendered(interrupted.display_lines(/*width*/ 80)), @r#"
    • Called server.lookup({"title":"A"})
      └ Error: interrupted
    • Called server.lookup({"title":"B"})
      └ Error: interrupted
...
fn node_repl_group_transcript_preserves_expanded_detail() {
    ...
    insta::assert_snapshot!(rendered(group.transcript_lines(/*width*/ 80)), @r#"
