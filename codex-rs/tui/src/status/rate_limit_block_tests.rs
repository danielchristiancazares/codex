...
fn snapshot_mapping_preserves_exhaustive_block_reasons() {
    ...
    assert_eq!(
        WorkspaceAccessState::from_snapshot(&snapshot(
            /*rate_limit_reached_type*/ None, /*spend_control_reached*/ None
        )),
...
fn rolling_spend_updates_do_not_replace_a_known_rate_limit_reason() {
    ...
    assert_eq!(
        blocked.merge_rolling(&snapshot(/*rate_limit_reached_type*/ None, Some(true))),
        blocked,
    );
    assert_eq!(
        blocked.merge_rolling(&snapshot(
            /*rate_limit_reached_type*/ None,
            Some(false)
        )),
        blocked,
