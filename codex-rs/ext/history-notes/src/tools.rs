...
impl HistoryNotesToolOutput {
    fn new(mut result: Value) -> Result<Self, FunctionCallError> {
        ...
        if let Some(images) = images {
            ...
            if images.len() > MAX_FUNCTION_OUTPUT_IMAGES {
                return Err(FunctionCallError::RespondToModel(format!(
                    "History returned more than {MAX_FUNCTION_OUTPUT_IMAGES} images; retry with narrower bounds"
                )));
            }
            ...
            for image in images {
                ...
                encoded_image_bytes = encoded_image_bytes.saturating_add(image_url.len());
                if encoded_image_bytes > MAX_FUNCTION_OUTPUT_IMAGE_ENCODED_BYTES {
                    return Err(FunctionCallError::RespondToModel(
                        "History returned more than 8 MiB of encoded images; retry with narrower bounds"
                            .to_string(),
                    ));
                }
                content.push(FunctionCallOutputContentItem::InputImage { image_url, detail });
            }
