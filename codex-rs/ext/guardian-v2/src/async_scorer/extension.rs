...
use super::assembled_context::PreparedModelContext;
use super::assembled_context::prepare_model_context;
...
impl GuardianV2Extension {
    ...
    async fn score_tool(&self, input: ToolStartInput<'_>) {
        ...
        tokio::spawn(async move {
            ...
            let transcript = match guardian_config.transcript.build_context_excluding_call(
                ContextTarget::Async,
                history.as_ref(),
            ...
            let trusted_review_evidence = sync_reviews
                ...
                .collect();
            let prepared_context = match prepare_model_context(
                transcript.authorization,
                transcript.entries,
                transcript.required_entry_indices,
                action,
                guardian_config.max_action_tokens,
            ) {
                Ok(context) => context,
                Err(error) => {
                    Self::record_fail_closed_score(thread.thread_extension_data(), sampled_at);
                    ...
                    event_sink.emit_warning(ExtensionWarning {
                        thread_id,
                        turn_id: Some(turn_id),
                        message: format!("Guardian V2 context assembly failed: {error}"),
                    });
                    return;
                }
            };
            let PreparedModelContext {
                items: classification_input,
                action:
                    RenderedAction {
                        text: planned_action,
                        original_bytes: action_original_bytes,
                    },
                original_bytes: context_original_bytes,
                retained_bytes: context_retained_bytes,
            } = prepared_context;
            truncations.record("action", action_original_bytes, planned_action.len());
            truncations.record(
                "assembled_context",
                context_original_bytes,
                context_retained_bytes,
            );
