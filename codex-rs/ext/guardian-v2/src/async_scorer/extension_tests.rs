...
async fn contributor_bounds_oversized_actions_and_fairly_truncates_nested_fields() -> Result<()> {
    ...
    let action_index = content.len() - 2;
    let action_text = content[action_index]["text"]
        .as_str()
    ...
    let serialized_context_bytes = serde_json::to_vec(&request["input"][2])?.len();
    assert!(
        serialized_context_bytes <= max_context_bytes,
        "the complete model-visible context item must remain bounded"
    );
    assert!(action_text.ends_with('\n'));
    assert!(
        action_text.len() <= max_context_bytes,
        "the raw model-visible action must remain bounded"
    );

    let mut placeholder_context = request["input"][2].clone();
    let placeholder_item_bytes = {
        let placeholder_content = placeholder_context["content"]
            .as_array_mut()
            .expect("placeholder content should remain an array");
        placeholder_content[action_index]["text"] = json!("\n");
        serde_json::to_vec(&placeholder_content[action_index])?.len()
    };
    let placeholder_context_bytes = serde_json::to_vec(&placeholder_context)?.len();
    let fixed_context_bytes = placeholder_context_bytes - placeholder_item_bytes;
    let available_action_item_bytes = max_context_bytes - fixed_context_bytes;
    let action_item_bytes = serde_json::to_vec(&content[action_index])?.len();
    assert!(action_item_bytes <= available_action_item_bytes);
    assert!(
        action_item_bytes >= available_action_item_bytes * 9 / 10,
        "water-filling should use the available JSON-encoded action budget"
    );
    assert_eq!(action["tool"], "read_file");
