...
async fn npm_pack_helper_does_not_allocate_console() -> anyhow::Result<()> {
    ...
    if std::env::var_os(PARENT).is_none() {
        ...
        parent.args(["--exact", "npm_source::windows_console_tests::npm_pack_helper_does_not_allocate_console", "--nocapture"])
...
"#)?;
    ...
    std::fs::write(&wrapper, format!(r#"@"{}" -NoProfile -NonInteractive -File "{}"
"#, powershell.display(), script.display()))?;
    super::pack_npm_package(temporary.path(), "fixture-package", None, None, wrapper.as_os_str()).map_err(anyhow::Error::msg)?;
