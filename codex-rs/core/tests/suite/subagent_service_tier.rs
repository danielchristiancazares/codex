...
fn request_service_tier(service_tier: ServiceTier) -> Option<&'static str> {
    match service_tier {
...
#[test_case(ServiceTier::Fast, None; "disabling fast mode updates active and idle child work")]
#[test_case(ServiceTier::Fast, Some(ServiceTier::Default); "explicit default updates active and idle child work")]
#[test_case(ServiceTier::Default, Some(ServiceTier::Fast); "enabling fast mode updates active and idle child work")]
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn root_service_tier_change_updates_existing_subagent(
    initial_service_tier: ServiceTier,
    updated_service_tier: Option<ServiceTier>,
) -> Result<()> {
    let server = start_mock_server().await;
    let initial_request_service_tier = request_service_tier(initial_service_tier);
    let updated_request_service_tier =
        request_service_tier(updated_service_tier.unwrap_or_default());
    let mut builder = test_codex()
        .with_model("gpt-5.6-sol")
        .with_config(move |config| {
            config.service_tier = initial_service_tier;
    ...
    assert_eq!(original_child_service_tier, initial_service_tier);
    ...
    .await;
    assert_request_service_tier(&initial_child_request, initial_request_service_tier);

    submit_thread_settings(
        &test.codex,
        ThreadSettingsOverrides {
            service_tier: Some(updated_service_tier),
            ..Default::default()
        },
    )
    .await?;
