...
async fn executor_skill_tool_reads_references_under_current_permissions(
    ...
    if matches!(read, ExecutorReferenceRead::Denied) {
        ...
        if matches!(read, ExecutorReferenceRead::DeniedContinuation) {
            ...
            const MAX_REFERENCE_PAGES: usize = 16;
            let mut collected = first_contents.to_string();
            let mut reached_eof = false;
            for page_index in 1..MAX_REFERENCE_PAGES {
                let response_id = format!("resp-reference-{page_index}");
                let done_response_id = format!("{response_id}-done");
                let call_id = format!("continue-reference-{page_index}");
                let response = responses::mount_sse_sequence(
                    &server,
                    vec![
                        sse(vec![
                ...
                let page_contents = page["contents"]
                    .as_str()
                    .expect("continuation page contents");
                let next_len = collected.len().saturating_add(page_contents.len());
                assert!(next_len <= contents.len(), "continuation exceeded snapshot");
                assert_eq!(page_contents, &contents[collected.len()..next_len]);
                assert_eq!(page["resource"].as_str(), Some(resource.as_str()));
                let expected_skill_root = skill_dir.inferred_native_path_string();
                assert_eq!(
                    page["skill_root"].as_str(),
                    Some(expected_skill_root.as_str())
                );
                collected.push_str(page_contents);
                match page["next_cursor"].as_str() {
                    Some(next_cursor) => cursor = next_cursor.to_string(),
                    None => {
            ...
            assert!(reached_eof, "reference pagination exceeded page bound");
