...
async fn grandchild_full_fork_preserves_context_baseline(
    ...
    .await;
    // Descendant completion can arrive after a parent's post-tool response and
    // require another sampling continuation. Bound replies to followups to
    // the recorded spawn calls, so initial requests still need their own mocks.
    for sequence in 0..8 {
        mount_sse_once_match(
            &server,
            |req: &wiremock::Request| {
            ...
            sse(vec![ev_completed(&format!(
                "baseline-parent-finished-{sequence}"
            ))]),
        )
        .await;
    }
    ...
    for (mock, agent_name) in [
        ...
        .await
        .with_context(|| format!("waiting for context-baseline request from {agent_name}"))?;
        ...
        let mut last_status = thread.agent_status().await;
        timeout(Duration::from_secs(/*secs*/ 10), async {
            while !matches!(last_status, AgentStatus::Completed(_)) {
                sleep(Duration::from_millis(/*millis*/ 10)).await;
                last_status = thread.agent_status().await;
            }
        })
        .await
        .with_context(|| {
            format!("waiting for context-baseline completion from {agent_name}: {last_status:?}")
        })?;
