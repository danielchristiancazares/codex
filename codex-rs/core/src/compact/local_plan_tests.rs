...
fn context_rejection_reduces_multiple_complete_turn_groups_once() {
    ...
    let before = plan.prompt_input(&default_input_modalities());

    let reduction = plan
        .reduce_after_context_error(&base_instructions(), RequestBudget::ProviderEnforced)
        .expect("the plan should remove old turns");
    let after = plan.prompt_input(&default_input_modalities());

    assert!(reduction.removed_groups >= 2);
    assert!(reduction.removed_items >= 4);
    assert_eq!(&after[after.len() - 3..], &before[before.len() - 3..]);
    assert_eq!(
        plan.reduce_after_context_error(&base_instructions(), RequestBudget::ProviderEnforced),
...
fn replacement_budget_truncates_an_oversized_summary() {
    ...
    let replacement = plan
        .build_replacement(
            ...
            &summary,
            &base_instructions(),
            RequestBudget::ClientEnforced(200),
            CompactedMessageIdentity::Regenerate,
        )
        .expect("the summary should be reduced to fit");

    assert!(replacement.summary_text.0.len() < summary.len());
