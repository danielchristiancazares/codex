...
async fn run_compact_task_inner_impl(
    ...
    let compaction_output = loop {
        let turn_input = plan.prompt_input(&turn_context.model_info().input_modalities);
        let prompt = Prompt {
            input: turn_input,
        ...
        match attempt_result {
            ...
            Err(e) if matches!(e.details(), CodexErrorDetails::ContextWindowExceeded) => {
                if let Ok(reduction) = plan.reduce_after_context_error(
                    &base_instructions,
                    RequestBudget::for_model(turn_context.model_info()),
                ) {
                    error!(
    ...
    let compaction_response_id = compaction_output.response_id.into_string();
    let summary_suffix =
    ...
    let (initial_context, _) = sess
        .rehydrate_additional_context_for_compaction(initial_context)
        .await;
    ...
    let summary_text = replacement.summary_text.into_string();
    ...
    let new_history = sess
        .fit_compaction_replacement(&turn_context, new_history)
        .await?;
