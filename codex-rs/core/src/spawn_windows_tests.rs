use super::{SpawnChildRequest, StdioPolicy, spawn_child_async};
...
fn console_probe() -> std::io::Result<()> {
    let path = std::env::var_os("CODEX_SPAWN_CONSOLE_REPORT").ok_or_else(|| std::io::Error::other("report path"))?;
    // SAFETY: This only queries this process's console association.
    std::fs::write(path, if unsafe { GetConsoleWindow() }.is_null() { "none" } else { "attached" })?;
...
async fn exercise(name: &str, policy: StdioPolicy) -> anyhow::Result<()> {
    ...
    if std::env::var_os("CODEX_SPAWN_CONSOLE_PARENT").is_none() {
        ...
        command.args(["--exact", &format!("spawn::windows_console_tests::{name}"), "--nocapture"])
            .env("CODEX_SPAWN_CONSOLE_PARENT", "1").creation_flags(0x0000_0008).kill_on_drop(true);
        let output = tokio::time::timeout(Duration::from_secs(20), command.output()).await??;
        assert!(output.status.success(), "detached spawn test failed: {output:?}");
    ...
    env.insert("CODEX_SPAWN_CONSOLE_REPORT".to_owned(), report.to_string_lossy().into_owned());
    let child = spawn_child_async(SpawnChildRequest {
        program: exe, args: ["--exact", "spawn::windows_console_tests::console_probe", "--ignored", "--nocapture"].map(str::to_owned).to_vec(),
        arg0: None, cwd: temp.path().to_path_buf().try_into()?,
        network_sandbox_policy: codex_protocol::permissions::NetworkSandboxPolicy::Enabled,
        network: None, stdio_policy: policy, env,
    }).await?;
    ...
    let expected = match policy { StdioPolicy::RedirectForShellTool => "none", StdioPolicy::Inherit => "attached" };
...
#[tokio::test]
async fn redirected_shell_does_not_allocate_console() -> anyhow::Result<()> { exercise("redirected_shell_does_not_allocate_console", StdioPolicy::RedirectForShellTool).await }
#[tokio::test]
async fn inherited_stdio_preserves_console_behavior() -> anyhow::Result<()> { exercise("inherited_stdio_preserves_console_behavior", StdioPolicy::Inherit).await }
