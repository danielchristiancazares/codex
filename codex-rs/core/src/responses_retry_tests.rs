...
async fn transport_fallback_respects_server_retry_delay(request: ResponsesStreamRequest) {
    ...
    tokio::time::pause();
    let error = CodexErr::Stream("slow down".to_string())
        .with_retry_after(RetryAfter::from_delay(delay).expect("retry deadline"));
...
#[test_case::test_case(CodexErr::Stream("connection closed".to_string()); "no_advice")]
#[test_case::test_case(
    CodexErr::Stream("connection closed".to_string())
        .with_retry_after(RetryAfter::from_delay(Duration::ZERO).expect("retry deadline"));
    "zero_advice"
)]
#[tokio::test]
async fn transport_fallback_without_positive_advice_is_immediate(error: CodexErr) {
    let (session, turn_context, _events) =
        crate::session::tests::make_session_and_context_with_auth_and_config_and_rx(
            codex_login::CodexAuth::from_api_key("Test API Key"),
            Vec::new(),
...
async fn terminal_error_with_server_advice_never_switches_transport() {
    ...
    let error = super::handle_response_stream_error(
        &mut retry_state,
        /*max_retries*/ 0,
        CodexErr::QuotaExceeded.with_retry_after(
            RetryAfter::from_delay(Duration::from_secs(5)).expect("retry deadline"),
        ),
        &mut client_session,
        &session,
        &turn_context,
