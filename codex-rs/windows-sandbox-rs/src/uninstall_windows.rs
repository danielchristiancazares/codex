...
pub fn clean_up_packaged_windows_sandbox(
    ...
    let accounts_removed = account_results.iter().all(Result::is_ok);
    for result in account_results {
        if let Err(error) = result {
            errors.push(format!("{error:#}"));
        }
    }
    // Retain the canonical passwords when account deletion is incomplete so a repair can reuse
    // them. Once both accounts are absent, no later installation should inherit the old secret.
    if accounts_removed && let Err(error) = crate::machine_credentials::remove_machine_credentials()
    {
        errors.push(format!("{error:#}"));
    }
    if errors.is_empty() {
