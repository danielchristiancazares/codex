...
/// Decodes one JSONL rollout record through [`decode_rollout_line`].
pub fn decode_rollout_line_slice(bytes: &[u8]) -> serde_json::Result<RolloutLine> {
    serde_json::from_slice(bytes).and_then(decode_rollout_line)
}
