...
#[tokio::test]
async fn rollout_search_does_not_allocate_console() -> anyhow::Result<()> {
    ...
    if std::env::var_os(PARENT).is_none() {
        ...
        parent.args(["--exact", "search::windows_console_tests::rollout_search_does_not_allocate_console", "--nocapture"])
        ...
        assert!(output.status.success(), "detached helper regression failed: {output:?}");
...
"#)?;
    let system_root = std::env::var_os("SystemRoot").ok_or_else(|| anyhow::anyhow!("SystemRoot missing"))?;
    let powershell = std::path::PathBuf::from(system_root).join("System32/WindowsPowerShell/v1.0/powershell.exe");
    let wrapper = temporary.path().join("rg-probe.cmd");
    std::fs::write(&wrapper, format!(r#"@cd /d "%~dp0"
@"{}" -NoProfile -NonInteractive -File "{}"
"#, powershell.display(), script.display()))?;
    let result = super::ripgrep_rollout_paths(&wrapper, temporary.path(), "fixture").await?;
    assert!(result.is_some());
    pretty_assertions::assert_eq!(std::fs::read_to_string(temporary.path().join("console-state"))?, "none");
