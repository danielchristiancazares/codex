...
async fn exercise(name: &str) -> anyhow::Result<()> {
    ...
    if std::env::var_os(PARENT).is_none() {
        ...
        parent
            .args([
                "--exact",
                &format!("client_transport::windows_console_tests::{name}"),
                "--nocapture",
            ])
            .env(PARENT, "1")
        ...
        assert!(
            ...
            "executor console test failed: {output:?}"
        );
        assert!(
            String::from_utf8_lossy(&output.stdout).contains("executor-console-check-complete")
        );
        return Ok(());
    }
    ...
    let output = if name == "stdio_exec_server_starts_without_console" {
        ...
        super::stdio_command_process(&specification)
            .kill_on_drop(true)
            .output()
            .await?
    } else {
    ...
    assert!(
        String::from_utf8_lossy(&output.stdout).contains("executor-console=none"),
        "executor allocated a console: {output:?}"
    );
