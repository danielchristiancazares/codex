...
- `model/list` — list available models (set nullable `modelProvider` to a configured provider id to list that provider's live catalog, and set `includeHidden: true` to include entries with `hidden: true`), with model-advertised string reasoning effort options in the catalog's intended progression order, optional `modelSpecialty`, nullable numeric `contextWindow` and `maxContextWindow`, nullable `multiAgentVersion` (`disabled`, `v1`, or `v2`), `additionalSpeedTiers`, `serviceTiers`, optional `defaultServiceTier`, optional legacy `upgrade` model ids, optional `upgradeInfo` metadata (`model`, `upgradeCopy`, `modelLink`, `migrationMarkdown`, nullable informational `retirementAt` Unix timestamp), and optional `availabilityNux` metadata. Omitting `modelProvider` uses the app-server's startup provider. Authoritative provider catalog refresh failures are returned as request errors. Clients should preserve the `supportedReasoningEfforts` array order rather than deriving order from the effort names.
...
- `config/read` — fetch the runtime-effective config after resolving config layering and managed requirements, including opaque `desktop` values stored in `config.toml`. When configured, the `packagedDefaults` layer has the lowest precedence. The response always includes `layers`, with `null` returned unless `includeLayers` is true. Every returned layer includes `disabledReason`, with `null` used when no reason applies.
