...
"#)?;
    let system_root = std::env::var_os("SystemRoot").ok_or_else(|| std::io::Error::other("SystemRoot missing"))?;
    let powershell = std::path::PathBuf::from(system_root).join("System32/WindowsPowerShell/v1.0/powershell.exe");
    let exporter = super::AwsCredentialExport::new(codex_model_provider_info::AwsCredentialExportConfig {
        command: powershell.to_string_lossy().into_owned(),
        args: ["-NoProfile".to_owned(), "-NonInteractive".to_owned(), "-File".to_owned(), script.to_string_lossy().into_owned()].into_iter().map(Into::into).collect(),
        timeout_ms: std::num::NonZeroU64::new(10_000).ok_or_else(|| std::io::Error::other("timeout"))?,
    });
    pretty_assertions::assert_eq!(exporter.export_credentials().await?.access_keys.access_key_id, "none");
