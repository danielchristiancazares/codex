...
async fn header_helper_keeps_consoleless_parent_and_json_output() -> anyhow::Result<()> {
    ...
    if std::env::var_os(CHILD).is_none() {
        ...
        command.args(["--exact", TEST, "--nocapture"]).env(CHILD, "1")
            .creation_flags(0x0000_0008).kill_on_drop(true); // DETACHED_PROCESS
        let output = tokio::time::timeout(Duration::from_secs(30), command.output()).await??;
        assert!(output.status.success(), "HTTP header helper regression failed: {output:?}");
        assert!(String::from_utf8_lossy(&output.stdout).contains("http-helper-console-check-complete"));
        return Ok(());
    }
...
"#)?;
    let system_root = std::env::var_os("SystemRoot").ok_or_else(|| anyhow::anyhow!("SystemRoot is absent"))?;
    let powershell = std::path::PathBuf::from(system_root).join("System32/WindowsPowerShell/v1.0/powershell.exe");
    let command = format!(r#""{}" -NoLogo -NoProfile -NonInteractive -File "{}""#, powershell.display(), script.display());
