...
/// A `response.incomplete` terminal event: the server finished the response without
/// completing it (for example `max_output_tokens` or `content_filter`).
///
/// This is not a transport interruption, so it must not be retried as one.
...
/// A required Responses event that could not be decoded.
///
