...
impl GitHubCopilotAuth {
    /// Uses the default credential policy when no resolved runtime config is available.
    ...
    /// Uses the same resolved Codex home and credential policy as other CLI authentication.
    pub fn new_in(
        codex_home: &Path,
        http_client_factory: HttpClientFactory,
    ...
    fn new_with_parts(
        ...
        Self {
            credential_store: CopilotCredentialStore::new(codex_home, store_mode, keyring),
        ...
        let existing = match self.credential() {
            ...
            Err(error)
                if force
                    && matches!(
                        error.kind(),
                        GitHubCopilotAuthErrorKind::MalformedCredential
                            | GitHubCopilotAuthErrorKind::CredentialStore
                    ) =>
            {
                None
            }
